...
const TOOLS = [
  ...
  { name: 'plan_flow', description: '神龍: turn a natural-language goal into a flow — ordered steps, which existing tools/agents cover each (have), and which are MISSING (gaps to build). Returns `available` (registered agents/tools/workflows + built-in agent:browser-control). Saves it to the workflow store so it is viewable in the web cockpit (🗂 Flows); pass save:false to design without saving. gap controls self-extension: "ask" (default) surfaces gaps to build, "auto" auto-generates them, "off" plans with existing tools only. Also returns summary_text + diagram_mermaid/diagram_ascii so you can show the plan and confirm before running — no web cockpit needed. NOTE: tools your client connects (e.g. claude.ai\'s Gmail) are NOT visible here — register them with add_integration, or a UI-only service resolves to agent:browser-control. Designs, does not run — act on it with run_workflow.',
