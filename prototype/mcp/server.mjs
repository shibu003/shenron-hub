...
const TOOLS = [
  ...
  { name: 'plan_flow', description: '神龍: turn a natural-language goal into a flow — ordered steps, which existing tools/agents cover each (have), and which are MISSING (gaps to build). Returns `available` (registered agents/tools/workflows + built-in agent:browser-control). Saves it to the workflow store so it is viewable in the web cockpit (🗂 Flows); pass save:false to design without saving. gap controls self-extension: "ask" (default) surfaces gaps to build, "auto" auto-generates them, "off" plans with existing tools only. DISCOVER-FIRST: it researches the goal across all options (registered tools / APIs / free MCP / platforms like Apps Script / browser-control / generate) and surfaces blockers (no-API / ToS / license). If the goal is ambiguous or has blockers, it returns `clarify` (questions+options, mode:"clarify") INSTEAD of a plan — present those to the user, then call plan_flow again with their answers in `context.choices` (e.g. [{"question":"...","answer":"..."}]). Also returns summary_text + diagram_mermaid/diagram_ascii so you can show the plan and confirm before running — no web cockpit needed. NOTE: tools your client connects (e.g. claude.ai\'s Gmail) are NOT visible here — register them with add_integration, or a UI-only service resolves to agent:browser-control. Designs, does not run — act on it with run_workflow.',
    inputSchema: { type: 'object', properties: { goal: { type: 'string' }, save: { type: 'boolean' }, gap: { type: 'string', enum: ['off', 'ask', 'auto'] }, cost: { type: 'string', enum: ['free', 'paid_ok'], description: 'free (default): only $0-marginal options, paid steps surfaced as opt-in; paid_ok: may use paid tools, cost disclosed' } }, required: ['goal'] } },
